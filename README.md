...
## フェーズ5: マクロ相関シグナル(雇用統計×金・ドル・AI株)

`macro_pattern_analysis.py` + `macro_signal_notifier.py` + `macro_signal.yml` を使うと、
米雇用統計・金・ドル指数・AI株(NVDA)の月次の値動きパターンと、過去の翌月BTC値動きの
関係から、現在の環境に当てはまるパターンがあればLONG/SHORTシグナルを通知します。

### 判定ロジック

- **要因A**: 雇用統計(NFP)が直近3ヶ月平均と比べて上振れ/下振れだったか
- **要因B**: 金・ドル指数・AI株(NVDA)の当月トレンドから算出する簡易リスクセンチメント
  (NVDA上昇/金下落/ドル下落 のうち多数派が「リスクオン」)。4指標を独立にバケット分けすると
  サンプル数が薄くなりすぎるため、金・ドル・AI株は1つの合成スコアにまとめている
