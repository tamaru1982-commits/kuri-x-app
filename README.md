...
## フェーズ5: マクロ相関シグナル(雇用統計×金・ドル・AI株・S&P500・VIX・原油・FF金利)

`macro_pattern_analysis.py` + `macro_signal_notifier.py` + `macro_signal.yml` を使うと、
米雇用統計・金・ドル指数・AI株(NVDA)・S&P500・VIX(恐怖指数)・原油(WTI)・FF金利
(FOMC政策金利)の月次の値動きパターンと、過去の翌月BTC値動きの関係から、現在の環境に
当てはまるパターンがあればLONG/SHORTシグナルを通知します。
...
- **要因B**: 金・ドル指数・AI株(NVDA)・S&P500・VIX・原油・FF金利の当月の動きから
  ...
  各指標を独立にバケット分けするとサンプル数が薄くなりすぎるため、全て1つの
  合成スコアにまとめている
  - リスクオン方向(+1票): NVDA上昇 / S&P500上昇 / 金下落 / ドル下落 / VIX下落 / 原油下落
  - リスクオフ方向(-1票): 上記の逆
  - FF金利は前月比で上昇(利上げ)なら-1票、低下(利下げ)なら+1票、変化なしなら投票なし
  - (原油上昇を一律リスクオフ扱いにしているのは簡易的な割り切りです。実際は
    需要主導の原油高ならリスクオンとも解釈できます)
- 要因A×要因Bの4パターンについて、過去の「翌月BTC値動き」の平均・上昇確率・サンプル数を
  毎回最新データで再集計し、現在の環境が該当するパターンのサンプル数が十分(既定10件以上)かつ
  上昇/下落に偏っている(既定65%以上/35%以下)場合のみシグナルとして通知

`db_utils.py`に`source="macro_pattern"`として記録され、`confluence_checker.py`が
他のシグナルとの一致も検知します。

### セットアップ手順

1. `macro_pattern_analysis.py` / `macro_signal_notifier.py`をリポジトリ直下にアップロード
2. `macro_signal.yml`を`.github/workflows/macro_signal.yml`に配置(毎月8日に自動実行)
3. Secretsは既存のもの(`FRED_API_KEY`, `DISCORD_WEBHOOK_URL`)を共用可能。追加は不要

### 注意事項(重要)

- **暗号資産の価格データは2014年9月頃までしか遡れません**。「2012年まで」の分析はできず、
  取得できる最古のデータ(2014年9月〜)から集計しています
- 月次×4パターンでの集計のため、1パターンあたりのサンプル数はせいぜい数十件程度です。
  他のシグナル(テクニカル・X投稿・クジラ)と比べて統計的な裏付けは明確に弱く、
  **必ず参考情報として扱い、これだけで売買判断をしない**でください
- 「AI株」という括り自体が市場で意識されるようになったのはここ数年の話で、
  2014〜2021年頃のNVDAは半導体株としての値動きです。長期の相関は文脈が異なる点に注意してください
- 投資助言ではありません
